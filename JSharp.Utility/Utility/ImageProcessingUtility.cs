...
namespace JSharp.Utility.Utility
{
    public static class ImageProcessingUtility
    {
        ...
        /// <exception cref="UnreachableException"></exception>
        public static MorphShape GetStructuringElementType(ShapeType shapeType)
        {
            return shapeType switch
            {
                ShapeType.Rhombus => MorphShape.Cross,
                ShapeType.Rectangle => MorphShape.Rectangle,
                _ => throw new UnreachableException("Enum mapping exhausted without a match - non-exhaustive switch.")
            };
        }
        ...
        public static bool IsBinaryImage(int[] histogramData)
        {
            ...
            if (histogramData[0] > 0)
            {
                nonZeroCount++; // Check brightness level 0
            }

            if (histogramData[255] > 0)
            {
                nonZeroCount++; // Check brightness level 255
            }
