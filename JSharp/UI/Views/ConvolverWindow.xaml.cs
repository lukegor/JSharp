...
namespace JSharp.UI.Views
{
    ...
    public partial class ConvolverWindow : Window
    {
        public ConvolverWindow()
        {
            ...
            {
                if (DataContext is ConvolverWindowViewModel vm)
                {
                    vm.PropertyChanged += (_, e) =>
                    {
                        if (e.PropertyName == nameof(ConvolverWindowViewModel.CurrentKernel))
                        {
                            CurrentKernelText_TextChanged(vm.CurrentKernel);
                        }
                    };
                }
            };
            ...
            if (currentKernel == Kernels.Canny)
            {
                for (int i = 0; i <= 2; i++)
                {
                    for (int j = 0; j <= 2; j++)
                    {
                        ...
                        if (FindName(cellName) is TextBox tb)
                        {
