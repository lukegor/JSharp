...
> **Note (2026-09-24):** history rewritten to the Open allowlist (UI + single-image ops); full history lives in private Pro. Older clones may contain pre-split history.
...
> **Open ops + UI demo** — real single-image pipelines (38 ops). Folder batch, `validate`, measurements export, and CLI are **Pro (trial exe)**. Full walkthrough on request.
...
Recipes (`run`/`validate`), folder batch, and the `ops` listing ship in the Pro trial exe.
Contact for evaluation — single-image ops above all run here.
